...
module OpenStreetMap
	class Locations
		class << self
			...
			GLOBAL_BOUNDARIES = { s: 38.991089, n: 39.151158, w: -94.611648, e: -94.497210 }
			STEP = 0.005
			TIMEOUT = 120
			PAUSE = 15
			RETRIES = 4
					...
					(GLOBAL_BOUNDARIES[:w]...GLOBAL_BOUNDARIES[:e]).step(STEP) do |ew|
						...
						begin
							...
							overpass.query(query(boundaries)).slice_before { |result|	result[:type] == 'node'	}.each do |node_set|
								...
								streets = streets.select{ |way| ['footway','cycleway','path','service','track'].exclude? way[:tags][:highway] }
								...
								if lat_lon_duplicate.present?
									LOGGER.add 0, "WARN: Found multiple street pairs at same lat/lon. Found #{streets.first} & #{streets.last}, had #{street_duplicate.to_s}"
